...
def _is_trivial_tuple_expr(node: itir.Expr) -> bool:
    ...
    if cpm.is_let(
        node
    ):  # TODO: write test case. occurs in test_preconditioner.py::test_scan_stencils[_tridiagonal_init_v1-_tridiagonal_init_tmp_out-test_case1]
        return _is_trivial_tuple_expr(node.fun.expr) and all(
            _is_trivial_tuple_expr(arg) for arg in node.args
        )
    return False


# occurs in _tridiagonal
def _transform_trivial_tuple_expr(
    stmt: itir.Stmt, declarations: list[itir.Temporary], uids: eve_utils.UIDGenerator
):
    if not isinstance(stmt, itir.SetAt):
        return None
    if cpm.is_let(stmt.expr) and any(
        trivial_args := [_is_trivial_tuple_expr(arg) for arg in stmt.expr.args]
    ):
        simplified_args = [
            ...
            for arg, is_trivial in zip(stmt.expr.args, trivial_args, strict=True)
        ]
